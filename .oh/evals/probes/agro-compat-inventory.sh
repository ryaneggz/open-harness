...
# desc: every OH_* identifier in tracked files appears in .oh/compat-inventory.json with one of the four classifications, no non-obsolete entry is stale, and every alias-sla entry names its AGRO_* spelling
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
INVENTORY="$ROOT/.oh/compat-inventory.json"
...
found="$(git -C "$ROOT" ls-files -z \
  | grep -zv -e '^\.oh/tasks/' -e '^\.oh/compat-inventory\.json$' \
...
[[ -z "$missing" ]] || fails+=("OH_* identifiers in tracked files that the inventory does not classify: $missing")

stale="$(comm -13 <(printf '%s\n' "$found") <(printf '%s\n' "$non_obsolete") | paste -sd' ' -)"
[[ -z "$stale" ]] || fails+=("non-obsolete inventory entries no tracked file mentions: $stale")
...
echo "PASS: every OH_* identifier in tracked files is classified in .oh/compat-inventory.json, no non-obsolete entry is stale, alias-sla entries carry their AGRO_* spelling, and the epic's persisted legacy paths are inventoried" >&2
exit 0
