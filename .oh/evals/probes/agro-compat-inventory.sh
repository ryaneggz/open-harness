...
# desc: every OH_* identifier in tracked or untracked non-ignored files appears in .oh/compat-inventory.json with one of the four classifications, no non-obsolete entry is stale, and every alias-sla entry names its AGRO_* spelling
set -euo pipefail
...
echo "PASS: every OH_* identifier in tracked or untracked non-ignored files is classified in .oh/compat-inventory.json, no non-obsolete entry is stale, alias-sla entries carry their AGRO_* spelling, and the epic's persisted legacy paths are inventoried" >&2
exit 0
