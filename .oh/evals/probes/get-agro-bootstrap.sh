...
# desc: STATIC guard — `.oh/scripts/get-agro.sh` exists, is executable, downloads the prebuilt agro.js
...
SCRIPT="$ROOT/.oh/scripts/get-agro.sh"

if [ ! -f "$SCRIPT" ]; then
  echo 'SKIPPED .oh/scripts/get-agro.sh not present' >&2
  exit 2
fi
