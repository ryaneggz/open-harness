...
# desc: every test or probe that copies .oh/scripts/docker-compose.sh into a fixture also copies .oh/scripts/compat.sh, and the wrapper still refuses to run without its sibling
set -euo pipefail
...
done < <(grep -rlE "copyFileSync\(SCRIPT|cp \"\$WRAPPER\"" "$ROOT/.oh/scripts/__tests__" "$ROOT/.oh/evals/probes" 2>/dev/null | grep -v "$(basename "${BASH_SOURCE[0]}")" || true)

tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
mkdir -p "$tmp/.oh/scripts" "$tmp/.devcontainer"
printf 'services: {}\n' > "$tmp/.devcontainer/docker-compose.yml"
cp "$WRAPPER" "$tmp/.oh/scripts/docker-compose.sh"
if bash "$tmp/.oh/scripts/docker-compose.sh" --repo-dir "$tmp" --print-argv config >/dev/null 2>"$tmp/err"; then
