...
- `.oh/skills/delegate/SKILL.md@c879ad80` — step 4's resume rule (`pending` re-runs the task) and step 5b's requirement to write status, artifact references, and observed settings back to the graph before the next wave.
- `.oh/tasks/delegate-follow-up/delegate-graph.json@8ba02851` — the ledger this PR shipped, and the `reconciliation` block recording what it looked like when the review caught it.
- `.oh/tasks/delegate-follow-up/evidence.md@58a538da` — finding A-1/A-8 and the fix.
