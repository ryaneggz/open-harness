...
- **PLACEHOLDER — probe coverage gap for the stale-`completed` contract, repair in flight.** An independent reviewer found that deleting the entire stale-`completed` bullet at `SKILL.md:270-273` leaves **both probes green**, so that contract ships unpinned — while `prd.json` US-005 claims the probes pin the corrected contracts *"so that a future edit cannot silently reopen any of the four gaps"*. A separate bounded worker is repairing the probe now. **This gap is open. Nothing here should be read as saying it is fixed.** This paragraph is a placeholder and will be replaced with the result when that work returns.
