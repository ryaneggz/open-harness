...
- **Commits post-date the audited head, and the audit did not see them.** The `AUDIT-PASS` above describes `8ba02851`. Landing after it: `58a538da` (this file), `7ecb933c` (two pattern pages plus the regenerated knowledge index), `263b9d52` and later (eval record and `RESULTS.md`), and the ledger/evidence corrections from the re-review. None is an implementation change — the three implementation files are untouched since `c879ad80` — but the pattern pages and the index are content no audit run classified. Part of this is structural: an evidence doc cannot be covered by the audit that produces it, and committing an eval pin moves the head past the pin.
- **Cross-session resume of a `running` task is unreachable on this provider.** The ledger honestly records that the Agent surface exposes no durable worker id to persist. The consequence, which the re-review drew out: a `running` task resumed in a *new* session has no reference to reconnect to, so it lands in the ambiguity branch and blocks writes. Safe, but it means D4's reconnect branch is only reachable within one session here; across sessions only the ambiguity branch is live.
