# Ralph iteration — prime-rl-integration

You are one iteration of a Ralph loop implementing the `prime-rl-integration` task. The full plan is in `.oh/tasks/prime-rl-integration/prd.md` and the structured task list is in `.oh/tasks/prime-rl-integration/prd.json`. The loop calls you again until `progress.txt` contains a line `STATUS: COMPLETE`.
...
1. **Read context** — in this order:
   - `.oh/tasks/prime-rl-integration/prd.json` — find the lowest-`priority` story where `passes: false`. That is your story for this iteration.
   - `.oh/tasks/prime-rl-integration/progress.txt` — read the "Codebase Patterns" section at the top (if any) and the most recent few iterations to see what's been done.
   - `.oh/tasks/prime-rl-integration/critique.md` — if present, the critic findings the stories must satisfy.
   - If `.oh/tasks/prime-rl-integration/prd.md` contains `## Wiki Alignment`, read it before choosing the story. When `Impact: REQUIRED`, the relevant story must keep wiki updates aligned with the PRD and the recorded DeepWiki comparison.
...
2. **Verify branch** — your branch is `feat/623-prime-rl-integration` (per `prd.json` `branchName`). If `/ship-spec`'s Advisor launched you in an isolated worktree at `.oh/worktrees/feat/623-prime-rl-integration`, that directory already has the branch checked out — `cd` into it and skip the checkout below. Otherwise, if you are not on the branch:
   ```bash
   ...
   git checkout -b feat/623-prime-rl-integration "$SHIP_SPEC_REMOTE/${SHIP_SPEC_BASE:-development}" 2>/dev/null \
     || git checkout feat/623-prime-rl-integration
   ```
...
- **Never push** to `development` or `main`. Branch is `feat/623-prime-rl-integration`.
...
- PRD: `.oh/tasks/prime-rl-integration/prd.md`
- Structured stories: `.oh/tasks/prime-rl-integration/prd.json`
