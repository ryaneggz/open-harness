...
import { createSlackContext } from "./slack-context.js";
...
// createSlackContext is defined in ./slack-context.ts (extracted for testability).
...
const handler: MomHandler = {
	...
	async handleStop(channelId: string, slack: SlackBot): Promise<void> {
		...
		if (state?.running) {
			...
			state.runner.abort();
			const result = await slack.safePost({ channel: channelId, text: "_Stopping..._" });
			state.stopMessageTs = result.ts; // Save for updating later
		} else {
			await slack.safePost({ channel: channelId, text: "_Nothing running_" });
		}
	},
