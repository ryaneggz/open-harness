...
export async function startQuickTunnel(
  ...
  runSafe(execCmd(name, ["tmux", "kill-session", "-t", session], { user: "sandbox" }), {
    stdio: "pipe",
  });
  ...
  run(
    execCmd(name, ["tmux", "new-session", "-d", "-s", session, "bash", "-lc", launch], {
      user: "sandbox",
    }),
    { stdio: "pipe" },
  );
  ...
  while (Date.now() < deadline) {
    const log = captureSafe(execCmd(name, ["cat", logFile], { user: "sandbox" })) ?? "";
  ...
  const tail =
    captureSafe(execCmd(name, ["tail", "-n", "20", logFile], { user: "sandbox" })) ??
    "(log unavailable)";
  runSafe(execCmd(name, ["tmux", "kill-session", "-t", session], { user: "sandbox" }), {
    stdio: "pipe",
  });
