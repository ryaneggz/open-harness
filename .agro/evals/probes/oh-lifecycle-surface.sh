...
COMPOSE_SCRIPT="$ROOT/.agro/scripts/docker-compose.sh"

if [[ ! -f "$CLI" || ! -f "$LIFECYCLE" ]]; then
  echo "SKIPPED: not a source checkout (.agro/cli/src absent)" >&2
  exit 2
fi
...
while IFS= read -r file; do
  ...
  if [[ -n "$hits" ]]; then
    missing+=("S5: ${file#"$ROOT"/} drives a compose project directly ($(head -1 <<<"$hits")) — go through .agro/scripts/docker-compose.sh")
  fi
done < <(find "$ROOT/.agro/scripts" "$ROOT/.github/workflows" -type f \( -name '*.sh' -o -name '*.yml' \) 2>/dev/null | sort)

ts_argv="$(grep -rnE --exclude-dir=__tests__ '"docker"[[:space:]]*,[[:space:]]*\[?[[:space:]]*"compose"' "$ROOT/.agro/cli/src" 2>/dev/null || true)"
if [[ -n "$ts_argv" ]]; then
