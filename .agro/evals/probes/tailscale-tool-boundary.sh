...
CATALOG=".agro/cli/src/lib/tools/catalog.ts"
...
funnel=$(grep -rniE '\bfunnel\b' .agro/skills/t3 .devcontainer 2>/dev/null || true)
if [ -n "$funnel" ]; then
  missing+=("Funnel appears in .agro/skills/t3 or .devcontainer — Funnel is public exposure and the harness ships no Funnel command")
fi
