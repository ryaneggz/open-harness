...
FILE_HOOK="$ROOT/.agro/hooks/deny-secret-paths.sh"
...
B_OH=$(fixture b-oh "$(jq -nc '{tool_input:{file_path:"/home/sandbox/harness/.agro/config.json"}}')")
assert allow "$FILE_HOOK" "$B_JEST" "file guard falsely denied jest.config.js (segment anchor lost)"
assert allow "$FILE_HOOK" "$B_OH"   "file guard falsely denied .agro/config.json (segment anchor lost)"
...
D_OH=$(fixture d-oh "$(jq -nc '{tool_input:{command:"cat .agro/config.json"}}')")
...
assert allow "$CMD_HOOK" "$D_OH"   "command guard falsely denied .agro/config.json"
