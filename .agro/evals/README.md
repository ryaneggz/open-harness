# .agro/evals/ — Context fitness-function probe corpus
...
`.agro/tasks/context-fitness-evals/prd.md` for the full design
...
A probe is an executable shell script at `.agro/evals/probes/<id>.sh` with a
...
after the fact — `.agro/evals/probes/wiki-skill-impact-append-only.sh` takes
`WIKI_LEDGER_BASE`, `.agro/evals/probes/wiki-pattern-persistence.sh` takes
`WIKI_PERSISTENCE_BASE`, and `.agro/evals/probes/sandbox-registry.sh` takes
...
`.agro/scripts/sandbox-boot-smoke.sh` against a real container on a Linux runner.
...
| Column | Meaning |
|--------|---------|
| `probe` | probe id (`<id>` of `.agro/evals/probes/<id>.sh`) |
| `tier` | `A` |
...
`/eval` (`.claude/skills/eval/`) discovers `.agro/evals/probes/*.sh`, runs each, and
...
- **Triggers.** Runs on every pull request and on every push to
  ...
  `.agro/evals/**` is one of those filters, so probe and `RESULTS.md` edits gate
  themselves.
- **Git read-only.** The runner rewrites `.agro/evals/RESULTS.md` in the writable
