...
- `.agro/scripts/reserve-github-release.mjs` — `releaseTagName` (the only `v` site), `attemptCreate`.
- `.agro/scripts/verify-release-aliases.sh` — `check`: two tags share one manifest digest.
- `.agro/scripts/promote-release-latest.sh` — canonical-branch check; digest-equal `latest` promotion.
