...
(`.agro/skills/eval/run.sh:70`) yet excludes SKIPPED from both failure branches
(`.agro/skills/eval/run.sh:76`, `.agro/skills/eval/run.sh:79`) and from the pass-rate
(`.agro/skills/eval/run.sh:112`), so the suite exits 0 while the subject goes
unexercised.
...
performing it once by hand. `.agro/evals/probes/wiki-skill-impact-append-only.sh:34-37`
