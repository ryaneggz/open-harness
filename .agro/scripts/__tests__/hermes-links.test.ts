...
const script = resolve(".agro/scripts/link-providers.sh");
const roots: string[] = [];
...
function fixture() {
  ...
  roots.push(root);
  mkdirSync(join(root, ".agro/skills/git"), { recursive: true });
  writeFileSync(join(root, ".agro/skills/git/SKILL.md"), "canonical");
  return root;
}
...
describe("Hermes-only additive linking", () => {
  it("requires integration without requiring Claude scaffolding", () => {
    ...
    expect(readlinkSync(slot(root))).toBe("../../.agro/skills");
  ...
  it("normalizes an absolute link only to the same canonical pack", () => {
    ...
    symlinkSync(join(root, ".agro/skills"), slot(root));
    expect(run(root).status).toBe(0);
    expect(readlinkSync(slot(root))).toBe("../../.agro/skills");
  });
