...
- `.agro/cli/src/commands/harness.ts` sets the same target-local home before installation.
- `.agro/scripts/link-providers.sh` owns the additive `skills/openharness` link to `.agro/skills`. Installation reconciles it immediately; boot reuses that implementation.
...
- `.agro/install/banner.sh` reads project-local `auth.json` for authentication status.
...
```bash
OH_HERMES_SMOKE=1 bash .agro/scripts/hermes-install-smoke.sh
```
