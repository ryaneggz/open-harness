...
Convert `.agro/tasks/<short-desc>/prd.md` to `.agro/tasks/<short-desc>/prd.json` in the same folder.
...
Example: folder `.agro/tasks/install-prereq-detection/` + `--issue 175` + `--prefix task` → `branchName: "task/175-install-prereq-detection"`.
...
Archive fires only when **all** of the following hold for `.agro/tasks/<short-desc>/`:
...
2. Create `.agro/tasks/<short-desc>/archive/<ISO-timestamp>/` where the timestamp is `YYYY-MM-DDTHH-MM-SS` (UTC; second-resolution avoids same-day collisions).
...
- [ ] Operating on `.agro/tasks/<short-desc>/` — folder name is the feature name
- [ ] **Archive on rerun:** if `prd.json` exists AND `progress.txt` has run history, archived to `.agro/tasks/<short-desc>/archive/<ISO-timestamp>/` before overwriting
