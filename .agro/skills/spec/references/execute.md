...
The **execute** node is pointed at a planned `.agro/tasks/<slug>/` folder whose
...
Precondition: `.agro/tasks/<slug>/` carries the three-file contract (`prd.md`,
...
**Approving the plan is the commitment gate** (`.agro/skills/spec/SKILL.md`). If
...
```bash
jq -e 'all(.userStories[]; .passes == true)' ".agro/tasks/<slug>/prd.json"   # 0 => not RUNNING
```
...
2. Determine what moved between the planning base and the execution base:
   ...
   ```bash
   PLAN_BASE=$(sed -n 's/^- \*\*Base commit\*\*: `\(.*\)`.*/\1/p' ".agro/tasks/<slug>/prd.md" | head -1)
...
planner/proposer's input (`.agro/skills/wiki/references/schema.md` § 3). If the
...
`<prefix>: <slug-as-prose>` per `.agro/skills/git/SKILL.md`:
...
gh issue create \
  ...
  --body-file <(printf '%s\n' \
    ...
    "" \
    "## PRD" \
    "- .agro/tasks/<slug>/prd.md (this branch)" \
    "" \
...
`.agro/tasks/` is gitignored, so the `-f` on that `git add` is load-bearing: a bare
`git add .agro/tasks/<slug>/` stages nothing and the scaffold commit silently omits
the contract. (`.agro/knowledge/` is **not** gitignored — knowledge pages take a
plain `git add`.)
...
2. Assign each dependency-ready story to a bounded `/delegate` worker with the
   complete dispatch record that `.agro/skills/delegate/SKILL.md` requires.
...
4. Do not load the pattern set. Patterns are the planner's input
   (`.agro/skills/wiki/references/schema.md` § 3); the owner re-reads the
...
it blocks. The owner writes the findings to `.agro/tasks/<slug>/simplicity-review.json`
in the schema and content-head rule that `.agro/skills/audit/references/implementation.md`
...
```bash
COUNTER=".agro/tasks/<slug>/simplify-rounds.json"
ROUNDS=$(jq -r '.rounds // 0' "$COUNTER" 2>/dev/null || echo 0)
PREV=$(jq -r '.netAdded // empty' "$COUNTER" 2>/dev/null)
NET=$(AUDIT_ROOT="$PWD" bash .agro/skills/audit/scripts/implementation-gates.sh \
...
reads `.agro/tasks/<slug>/ui-evidence.json` for `HEAD` and fails closed without it.
...
```bash
bash .agro/skills/eval/run.sh ; rc=$?
cat > ".agro/tasks/<slug>/eval-result.json" <<EOF
{
...
EOF
git add -f ".agro/tasks/<slug>/eval-result.json"
```
...
bash .agro/skills/wiki/scripts/knowledge-impact.sh \
...
`.agro/knowledge/` is tracked). If `Expected Knowledge Impact` was `REQUIRED` and
...
1. **compound** — run `/wiki compile` on step 8's retro report to turn its
   supported lessons into `kind: pattern` pages under `.agro/knowledge/patterns/`,
...
Then **stop**. The human owns the merge (`.agro/skills/spec/SKILL.md`: human merge
...
### Branch + commit conventions (from `.agro/skills/git/SKILL.md`)
...
| Primitive | Path | Role |
|---|---|---|
| Task prompt template | `.agro/skills/spec/templates/task-prompt.md` | Step 4 — rendered at execution time; never persisted |
| `/worktrees` skill | `.agro/skills/worktrees/SKILL.md` | Step 4 — isolated `.worktrees/<branch>` for the implementation |
| `/delegate` skill | `.agro/skills/delegate/SKILL.md` | Step 4 — the bounded workers that perform tracked implementation edits beneath the one owner |
| `/audit implementation` | `.agro/skills/audit/SKILL.md` | Step 5 — the per-unit verdict gate |
...
| `/benchmark` skill | `.agro/skills/benchmark/SKILL.md` | Step 9 — the progress-ceiling verdict |
| `/audit pr` skill | `.agro/skills/audit/SKILL.md` | Step 10 — promotable classification (gates the undraft) |
| `/ci-status` skill | `.agro/skills/ci-status/SKILL.md` | CI verification (subsumed by the PR audit's promotable check) |
| Protected-paths list | `.claude/protected-paths.txt` | Load-bearing items a spec must not propose deleting |

## Pipeline position

Within the workflow owned by `.agro/skills/spec/SKILL.md`, `execute` is the
