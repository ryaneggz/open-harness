...
The wiki is a persistent, LLM-maintained knowledge base that lives in `workspace/wiki/`. Unlike the [memory system](/docs/guide/identity), which tracks operational decisions and lessons learned from doing work, the wiki stores **structured domain knowledge** extracted from external source documents — articles, papers, transcripts, notes.
