...
`@openharness/sandbox` contains the Docker Compose command builders and tool definitions. Each tool (quickstart, run, stop, clean, shell, etc.) constructs and executes Docker commands against the `.devcontainer/` configuration.
