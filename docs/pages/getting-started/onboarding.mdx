...
- Runs `gh auth setup-git` to configure git credential helper
- Eliminates the need for SSH keys for git operations
- Works with HTTPS remotes automatically
...
- Runs `pi` to trigger OAuth authentication
- Pi Agent powers the Slack bot, heartbeats, and extensions
- Required if using any automation features (Slack, scheduled tasks)
