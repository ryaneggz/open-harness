...
- **Command guard:** [`.agro/hooks/deny-env-dump.sh`](../.agro/hooks/deny-env-dump.sh) (PreToolUse `Bash`) is a two-tier scanner over the raw command string:
  ...
  - **Deny (operator-only dir)** — *any* command naming the `.config/` directory as a path segment, at the repo root or in `$HOME`. This tier is verb-agnostic on purpose: the operator owns the directory outright, so read, write, traversal, and archive routes are all closed rather than enumerated (a verb allowlist leaks through `python`/`node`/`perl`/`tar` and every tool added later). Anchored to a whole path segment, so `jest.config.js`, `--config foo`, `git config`, and `.agro/config.json` are unaffected.
...
- **File-path guard:** [`.agro/hooks/deny-secret-paths.sh`](../.agro/hooks/deny-secret-paths.sh) (PreToolUse `Read|Write|Edit|NotebookEdit|Grep|Glob`) blocks the same credential-path family for the file tools, mirroring the deny globs, and denies the operator-only `.config/` directory for both read and write. It scans every path-shaped field of `tool_input` (`file_path`, `notebook_path`, `path`, `glob`) so `Grep`/`Glob` cannot walk into a denied directory that `Read` is blocked from; Grep's `pattern` is a content regex, not a path, and is deliberately not scanned.
...
- **Non-blocking warn:** [`.agro/hooks/warn-devtcp.sh`](../.agro/hooks/warn-devtcp.sh) prints a stderr warning (never blocks) when a command uses `/dev/tcp` or `/dev/udp`.
...
**not** scan for secret leakage, and the four `.agro/hooks/` guards remain
...
- **Caveat 4 — the optional sshd overlay (RECOMMENDED to configure).** The base
  ...
  enabled via `access.ssh: true` in `agro.json`) starts `sshd` and ships a **safe
